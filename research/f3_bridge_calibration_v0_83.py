#!/usr/bin/env python3
"""v0.83: equal-budget proof-aware calibration of three NEW v0.82 F3s.

Does not harvest. Does not resume g141/g158 or exhausted v0.80/v0.81 F4/F5
roots. Canonical 172 is not a search input.
"""

from __future__ import annotations

import json
import sys
import time
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

from spider.f3_bridge_calibration import (
    CONTINUATION_RESERVE_S,
    G141_FIRST_VIABLE_S,
    N_ROOTS,
    ROOT_SPECS,
    STAGE_A_S,
    STAGE_A_UNIQUE,
    TOTAL_S,
    choose_calibration_verdict,
    equal_f_composition_differs,
    next_recommendation,
    select_calibration_portfolio,
    verify_all_calibration_roots,
)
from spider.f3_quality_frontier import (
    G141_CONTROL,
    G158_CONTROL,
    aggregate_f3_probes,
    bridge_loss,
    is_known_closed,
    load_closed_root_table,
    next_foundation_quality,
    probe_proof_aware_target,
)
from spider.g128_focused_endgame import reconstruct_g128_root
from spider.proof_aware_tactical_bridge import recover_digest_path
from spider.f3_tactical_bridge import BRIDGE_CEILING, search_f4_portfolio
from spider.integrated_policy import AUTONOMOUS_INCUMBENT_MW, CANDIDATE_CEILING, verify_autonomous_192
from spider.metrics import CANONICAL_MW_COST, RECORD_MW_COST, replay_actions
from spider.proof_aware_tactical_bridge import interpret_continuation_stop
from spider.research_actions import as_actions, is_deal
from spider.solution_forensics import load_opening
from spider.state_convergence import foundation_progression
from spider.whole_game_epoch_scheduler import SEARCH_RSS_MB, SEARCH_UNIQUE, save_solution

EXPERIMENT = "f3_bridge_calibration_v0_83"
BASE_SHA = "a386ebe2a82f59b4f2162cb0ac0b1f88d2426413"
BRANCH = "agent/f3-bridge-calibration-v0-83"
RESULT = ROOT / "docs" / "research" / f"{EXPERIMENT}.json"
REPORT = ROOT / "docs" / "research" / f"{EXPERIMENT}.md"
PROG = ROOT / "docs" / "research" / "f3_bridge_calibration_progress_v0_83.json"
FIX = ROOT / "solutions" / "4925153_autonomous_v0_83.moves"
META = ROOT / "solutions" / "4925153_autonomous_v0_83.json"


def _write_json(path: Path, payload: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temp = path.with_suffix(path.suffix + ".tmp")
    temp.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    temp.replace(path)


def _jsonable(obj):
    if obj is None or isinstance(obj, (str, int, float, bool)):
        return obj
    if isinstance(obj, dict):
        return {str(k): _jsonable(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [_jsonable(v) for v in obj]
    return obj


def _fmt(x, nd=1):
    if x is None:
        return "—"
    if isinstance(x, float):
        return f"{x:.{nd}f}"
    return str(x)


def slim_root(r: dict) -> dict:
    return {k: r.get(k) for k in (
        "calibration_name", "g", "foundations", "assembly_h", "assembly_f", "slack",
        "face_down", "empty_n", "legal_tableau", "boundaries", "ready_suits",
        "n_ready", "ordered_digest", "ident", "ok", "reason",
    )}


def slim_term(r: dict) -> dict:
    return {k: r.get(k) for k in (
        "g", "foundations", "assembly_h", "assembly_f", "slack", "quality",
        "bridge_loss", "root_g", "root_h", "root_f", "tactical_target",
        "legal_tableau", "boundaries", "class", "viable", "closed", "closed_tag",
        "ordered_digest", "ident", "delta_g", "delta_h", "delta_f",
        "assembly_payback", "n_actions",
    )}


def slim_probe(pr: dict) -> dict:
    keep = (
        "suit", "stage", "operational_rank", "elapsed_s", "unique", "expanded",
        "generated", "stop_reason", "raw_count", "viable_count", "surplus_count",
        "cheapest_viable_g", "best_viable_f", "min_h_viable", "max_viable_slack",
        "first_viable_s", "min_cover", "min_blockers", "proof_prunes",
    )
    rec = {k: pr.get(k) for k in keep}
    rec["viable_terminals"] = [slim_term(t) for t in (pr.get("viable_terminals") or [])[:4]]
    return rec


def slim_agg(agg: dict) -> dict:
    return {
        "name": agg.get("calibration_name"),
        "root_g": agg.get("root_g"),
        "root_h": agg.get("root_h"),
        "root_f": agg.get("root_f"),
        "root_slack": agg.get("root_slack"),
        "root_empty_n": agg.get("root_empty_n"),
        "root_legal": agg.get("root_legal"),
        "root_boundaries": agg.get("root_boundaries"),
        "ready_suits": agg.get("ready_suits"),
        "raw_count": agg.get("raw_count"),
        "viable_count": agg.get("viable_count"),
        "surplus_count": agg.get("surplus_count"),
        "strong_surplus_count": agg.get("strong_surplus_count"),
        "best_terminal_f": agg.get("best_terminal_f"),
        "best_slack": agg.get("best_slack"),
        "cheapest_terminal_g": agg.get("cheapest_terminal_g"),
        "lowest_terminal_h": agg.get("lowest_terminal_h"),
        "bridge_loss": agg.get("bridge_loss"),
        "best_payback": agg.get("best_payback"),
        "quality": agg.get("quality"),
        "elapsed_s": agg.get("elapsed_s"),
        "unique": agg.get("unique"),
        "proof_prunes": agg.get("proof_prunes"),
        "probes": [slim_probe(p) for p in agg.get("probes") or []],
        "viable_terminals": [slim_term(t) for t in (agg.get("viable_terminals") or [])[:8]],
        "ordered_digest": agg.get("ordered_digest"),
        "ident": agg.get("ident"),
    }


def write_report(p: dict) -> None:
    lines = [
        f"# {EXPERIMENT}",
        "",
        f"Verdict: `{p.get('verdict')}`",
        "",
        p.get("verdict_reason") or "",
        "",
        "Equal-budget proof-aware calibration of three NEW v0.82 Pareto F3s. "
        "No F3 harvest. No g141/g158 search. No exhausted v0.80/v0.81 F4/F5 roots. "
        f"Stage A {STAGE_A_S:.0f}s/target exceeds the g141 first-viable time "
        f"({G141_FIRST_VIABLE_S}s). Canonical 172 was not a search input.",
        "",
        "## Roots",
        "",
        "| name | g | h | f | slack | fd | empties | legal | boundaries | ok |",
        "| --- | -: | -: | -: | ----: | -: | ------: | ----: | ---------: | --- |",
    ]
    for r in p.get("roots") or []:
        lines.append(
            f"| {r.get('calibration_name')} | {r.get('g')} | {r.get('assembly_h')} | "
            f"{r.get('assembly_f')} | {r.get('slack')} | {r.get('face_down')} | "
            f"{r.get('empty_n')} | {r.get('legal_tableau')} | {r.get('boundaries')} | {r.get('ok')} |"
        )
    lines += [
        "",
        "## Frozen controls",
        "",
        f"g141: f={G141_CONTROL['f']} best next f={G141_CONTROL['best_next_f']} loss +{G141_CONTROL['bridge_loss']}",
        f"g158: f={G158_CONTROL['f']} best next f={G158_CONTROL['best_next_f']} loss +{G158_CONTROL['bridge_loss']}",
        "",
        "## Bridge-value table",
        "",
        "| name | root g | root h | root f | target | terminal F | terminal g | terminal h | terminal f | slack | bridge loss |",
        "| --- | -----: | -----: | -----: | ------ | ---------: | ---------: | ---------: | ---------: | ----: | ----------: |",
    ]
    for rec in p.get("bridge_table") or []:
        lines.append(
            f"| {rec.get('name')} | {rec.get('root_g')} | {rec.get('root_h')} | {rec.get('root_f')} | "
            f"{rec.get('target')} | {rec.get('terminal_F')} | {rec.get('terminal_g')} | "
            f"{rec.get('terminal_h')} | {rec.get('terminal_f')} | {rec.get('slack')} | "
            f"{rec.get('bridge_loss')} |"
        )
    if not p.get("bridge_table"):
        lines.append("| — | — | — | — | — | — | — | — | — | — | — |")
    lines += [
        "",
        "## Root/target probes",
        "",
        "| root g | root h | root f | target rank | target | stop | unique | exp | raw | viable | best g | best h | best f | slack |",
        "| -----: | -----: | -----: | ----------: | ------ | ---- | -----: | --: | --: | -----: | -----: | -----: | -----: | ----: |",
    ]
    for rec in p.get("probe_table") or []:
        lines.append(
            f"| {rec.get('root_g')} | {rec.get('root_h')} | {rec.get('root_f')} | "
            f"{rec.get('target_rank')} | {rec.get('target')} | {rec.get('stop')} | "
            f"{rec.get('unique')} | {rec.get('expanded')} | {rec.get('raw')} | {rec.get('viable')} | "
            f"{rec.get('best_g')} | {rec.get('best_h')} | {rec.get('best_f')} | {rec.get('slack')} |"
        )
    if not p.get("probe_table"):
        lines.append("| — | — | — | — | — | — | — | — | — | — | — | — | — | — |")
    lines += [
        "",
        "## g160 vs g161 at equal f=183",
        "",
        "| metric | g160/h23 | g161/h22 |",
        "| --- | ---: | ---: |",
    ]
    cmp_ = p.get("g160_vs_g161") or {}
    for key, label in (
        ("root_f", "root f"),
        ("empties", "empties"),
        ("legal", "legal"),
        ("boundaries", "boundaries"),
        ("best_next_F", "best next F"),
        ("best_next_g", "best next g"),
        ("best_next_h", "best next h"),
        ("best_next_f", "best next f"),
        ("bridge_loss", "bridge loss"),
        ("first_viable_s", "time to first viable"),
        ("proof_prunes", "proof prunes"),
    ):
        pair = cmp_.get(key) or {}
        lines.append(f"| {label} | {pair.get('g160', '—')} | {pair.get('g161', '—')} |")
    lines += [
        "",
        "## Bridge summary",
        "",
        "| root | best terminal | bridge loss | class | time |",
        "| ---- | ------------- | ----------: | ----- | ---: |",
    ]
    for rec in p.get("bridge_summary") or []:
        lines.append(
            f"| {rec.get('root')} | {rec.get('best_terminal')} | {rec.get('bridge_loss')} | "
            f"{rec.get('class')} | {_fmt(rec.get('time'))} |"
        )
    lines += [
        "",
        f"raw={p.get('n_raw')} viable={p.get('n_viable')} surplus1={p.get('n_surplus1')} "
        f"strong={p.get('n_strong')} closed_hits={p.get('n_closed')}",
        "",
        "## Continuation",
        "",
        f"portfolio n={p.get('n_portfolio')} sources={p.get('portfolio_sources')} "
        f"targets={p.get('portfolio_targets')} stop=`{p.get('stop_reason')}` "
        f"unique={p.get('unique')} exp={p.get('expanded')} generated={p.get('generated')} "
        f"maxF={p.get('max_foundations')} exhausted={p.get('roots_exhausted')}",
        "",
        p.get("continuation_note") or "",
        "",
        f"wall_s={_fmt(p.get('wall_s'))} reserve={CONTINUATION_RESERVE_S}s no_stage_b=True",
        "",
        "## Interpretation",
        "",
        p.get("interpretation") or "",
        "",
        "## Next recommendation",
        "",
        p.get("next_recommendation") or "",
        "",
    ]
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_text("\n".join(str(x) for x in lines) + "\n", encoding="utf-8")


def main() -> dict:
    opening, _raw, _labels = load_opening()
    wall0 = time.perf_counter()
    print("VERIFY autonomous 187", flush=True)
    inc = verify_autonomous_192(opening)
    if not inc.get("ok") or int(inc.get("g") or 0) != 187:
        payload = {
            "experiment": EXPERIMENT,
            "root_fail": True,
            "verdict": "F3_CALIBRATION_CONTRACT_FAILURE",
            "contract_reason": "incumbent 187 replay failed",
        }
        _write_json(RESULT, payload)
        write_report(payload)
        print("VERDICT F3_CALIBRATION_CONTRACT_FAILURE", flush=True)
        print("DONE", flush=True)
        return payload

    print("LOAD three NEW v0.82 F3s by metrics", flush=True)
    checked = verify_all_calibration_roots()
    for r in checked["roots"]:
        print(
            f"  {r.get('calibration_name')} ok={r.get('ok')} g={r.get('g')} h={r.get('assembly_h')} "
            f"f={r.get('assembly_f')} slack={r.get('slack')} ready={r.get('ready_suits')}",
            flush=True,
        )
    if not checked.get("ok"):
        payload = {
            "experiment": EXPERIMENT,
            "root_fail": True,
            "verdict": "F3_CALIBRATION_CONTRACT_FAILURE",
            "contract_reason": "calibration root verification failed",
            "roots": [slim_root(r) for r in checked["roots"]],
        }
        _write_json(RESULT, _jsonable(payload))
        write_report(payload)
        print("VERDICT F3_CALIBRATION_CONTRACT_FAILURE", flush=True)
        print("DONE", flush=True)
        return payload

    closed = load_closed_root_table()
    print(f"CLOSED roots n={len(closed)}; Stage A {STAGE_A_S}s/target (g141 first viable {G141_FIRST_VIABLE_S}s)", flush=True)
    _write_json(PROG, {"phase": "stage_a", "n_roots": N_ROOTS})

    stage_a = []
    for rec in checked["roots"]:
        targets = list(rec.get("remaining_targets") or [])[:4]
        print(
            f"PROBE {rec.get('calibration_name')} g={rec.get('g')} f={rec.get('assembly_f')} "
            f"targets={[t.get('suit') for t in targets]} cap={STAGE_A_S}s",
            flush=True,
        )
        probes = []
        for tgt in targets:
            remain = TOTAL_S - (time.perf_counter() - wall0)
            if remain < CONTINUATION_RESERVE_S + 5:
                print("PROBE skip: preserve continuation reserve", flush=True)
                break
            t_s = min(STAGE_A_S, remain - CONTINUATION_RESERVE_S)
            probe = probe_proof_aware_target(
                {"g": rec["g"], "ordered_digest": rec["ordered_digest"]},
                tgt,
                time_s=t_s,
                unique=STAGE_A_UNIQUE,
                stage="A",
            )
            probes.append(probe)
            print(
                f"  rank={tgt.get('operational_rank')} suit={tgt.get('suit')} "
                f"stop={probe.get('stop_reason')} unique={probe.get('unique')} "
                f"viable={probe.get('viable_count')} best_f={probe.get('best_viable_f')} "
                f"t={probe.get('elapsed_s'):.1f}s",
                flush=True,
            )
        agg = aggregate_f3_probes(rec, rec, probes)
        agg["calibration_name"] = rec.get("calibration_name")
        agg["inspect"] = rec
        stage_a.append(agg)
        print(
            f"  raw={agg.get('raw_count')} viable={agg.get('viable_count')} "
            f"best_f={agg.get('best_terminal_f')} loss={agg.get('bridge_loss')} "
            f"t={agg.get('elapsed_s'):.1f}s",
            flush=True,
        )

    all_aggs = list(stage_a)

    n_closed = 0
    screening_time_limited = False
    n_raw = n_viable = n_surplus1 = n_strong = 0
    best_new_f = None
    best_loss = None
    max_f = 3
    for agg in all_aggs:
        n_raw += int(agg.get("raw_count") or 0)
        n_viable += int(agg.get("viable_count") or 0)
        n_strong += int(agg.get("strong_surplus_count") or 0)
        if agg.get("best_terminal_f") is not None:
            bf = int(agg["best_terminal_f"])
            best_new_f = bf if best_new_f is None else min(best_new_f, bf)
            bl = agg.get("bridge_loss")
            if bl is not None:
                best_loss = int(bl) if best_loss is None else min(best_loss, int(bl))
        for rec in agg.get("viable_terminals") or []:
            max_f = max(max_f, int(rec.get("foundations") or 0))
            if next_foundation_quality(int(rec["assembly_f"])) == "SURPLUS_1":
                n_surplus1 += 1
            ident = rec.get("ident") or rec.get("whole_game_identity")
            if ident and is_known_closed(ident, int(rec["g"]), closed):
                n_closed += 1
                rec["closed"] = True
                rec["closed_tag"] = "KNOWN_CLOSED_STATE"
        if int(agg.get("root_slack") or 0) > 0:
            for pr in agg.get("probes") or []:
                if pr.get("stop_reason") == "time limit":
                    screening_time_limited = True

    bridge_table = []
    seen_rows = set()
    for agg in all_aggs:
        terms = agg.get("viable_terminals") or []
        if terms:
            best = min(terms, key=lambda r: (int(r["assembly_f"]), int(r["g"])))
            row = {
                "name": agg.get("calibration_name"),
                "root_g": agg.get("root_g"),
                "root_h": agg.get("root_h"),
                "root_f": agg.get("root_f"),
                "target": best.get("tactical_target"),
                "terminal_F": best.get("foundations"),
                "terminal_g": best.get("g"),
                "terminal_h": best.get("assembly_h"),
                "terminal_f": best.get("assembly_f"),
                "slack": best.get("slack"),
                "bridge_loss": bridge_loss(agg.get("root_f"), best.get("assembly_f")),
                "quality": next_foundation_quality(int(best["assembly_f"])),
            }
        else:
            live = None
            if agg.get("probes"):
                live = min(
                    agg["probes"],
                    key=lambda p: (
                        int(p.get("min_cover") if p.get("min_cover") is not None else 10**9),
                        -int(p.get("unique") or 0),
                    ),
                )
            row = {
                "name": agg.get("calibration_name"),
                "root_g": agg.get("root_g"),
                "root_h": agg.get("root_h"),
                "root_f": agg.get("root_f"),
                "target": None if live is None else live.get("suit"),
                "terminal_F": None,
                "terminal_g": None,
                "terminal_h": None,
                "terminal_f": None,
                "slack": None,
                "bridge_loss": None,
                "quality": None,
                "live_cover": None if live is None else live.get("min_cover"),
                "stop_reason": None if live is None else live.get("stop_reason"),
            }
        key = (row.get("name"), row.get("root_g"), row.get("target"), row.get("terminal_f"))
        if key in seen_rows:
            continue
        seen_rows.add(key)
        bridge_table.append(row)

    probe_table = []
    for agg in all_aggs:
        for pr in agg.get("probes") or []:
            best = None
            terms = pr.get("viable_terminals") or []
            if terms:
                best = min(terms, key=lambda r: (int(r["assembly_f"]), int(r["g"])))
            probe_table.append({
                "root_g": agg.get("root_g"),
                "root_h": agg.get("root_h"),
                "root_f": agg.get("root_f"),
                "target_rank": pr.get("operational_rank"),
                "target": pr.get("suit"),
                "stop": pr.get("stop_reason"),
                "unique": pr.get("unique"),
                "expanded": pr.get("expanded"),
                "raw": pr.get("raw_count"),
                "viable": pr.get("viable_count"),
                "best_g": None if best is None else best.get("g"),
                "best_h": None if best is None else best.get("assembly_h"),
                "best_f": None if best is None else best.get("assembly_f"),
                "slack": None if best is None else best.get("slack"),
            })

    def _side(agg: dict, key: str):
        if agg is None:
            return None
        mapping = {
            "root_f": agg.get("root_f"),
            "empties": agg.get("root_empty_n"),
            "legal": agg.get("root_legal"),
            "boundaries": agg.get("root_boundaries"),
            "best_next_F": None if not (agg.get("viable_terminals") or []) else max(int(t["foundations"]) for t in agg["viable_terminals"]),
            "best_next_g": agg.get("cheapest_terminal_g"),
            "best_next_h": agg.get("lowest_terminal_h"),
            "best_next_f": agg.get("best_terminal_f"),
            "bridge_loss": agg.get("bridge_loss"),
            "first_viable_s": None if not (agg.get("probes") or []) else next((p.get("first_viable_s") for p in agg["probes"] if p.get("first_viable_s") is not None), None),
            "proof_prunes": agg.get("proof_prunes"),
        }
        return mapping.get(key)

    by_g = {int(a.get("root_g")): a for a in all_aggs if a.get("root_g") is not None}
    a160, a161 = by_g.get(160), by_g.get(161)
    g160_vs_g161 = {
        k: {"g160": _side(a160, k), "g161": _side(a161, k)}
        for k in (
            "root_f", "empties", "legal", "boundaries", "best_next_F", "best_next_g",
            "best_next_h", "best_next_f", "bridge_loss", "first_viable_s", "proof_prunes",
        )
    }
    composition_differs = equal_f_composition_differs(a160, a161)
    bridge_summary = []
    for agg in all_aggs:
        if agg.get("best_terminal_f") is not None:
            term = f"F{max(int(t['foundations']) for t in (agg.get('viable_terminals') or [{'foundations': 4}]))} f={agg.get('best_terminal_f')}"
            cls = agg.get("quality")
        else:
            stops = sorted({p.get("stop_reason") for p in (agg.get("probes") or []) if p.get("stop_reason")})
            term = "none"
            cls = ",".join(str(s) for s in stops) or "no_probe"
        bridge_summary.append({
            "root": f"g{agg.get('root_g')}/h{agg.get('root_h')}/f{agg.get('root_f')}",
            "best_terminal": term,
            "bridge_loss": agg.get("bridge_loss"),
            "class": cls,
            "time": agg.get("elapsed_s"),
        })
    for ctrl, name in ((G141_CONTROL, "g141 control"), (G158_CONTROL, "g158 control")):
        bridge_summary.append({
            "root": name,
            "best_terminal": f"f={ctrl['best_next_f']}",
            "bridge_loss": ctrl["bridge_loss"],
            "class": "frozen",
            "time": None,
        })

    portfolio = select_calibration_portfolio(all_aggs, closed)
    cont = None
    remain = max(0.0, TOTAL_S - (time.perf_counter() - wall0))
    if portfolio and remain >= 5.0:
        print(f"CONTINUE n={len(portfolio)} t={remain:.1f}s", flush=True)
        _write_json(PROG, {"phase": "continuation", "n_portfolio": len(portfolio)})
        cont = search_f4_portfolio(opening, portfolio, time_s=remain, unique=SEARCH_UNIQUE)
        print(
            f"CONTINUE stop={cont.stop_reason} unique={cont.unique} exp={cont.expanded} "
            f"best={cont.solution_g} maxF={cont.max_foundations}",
            flush=True,
        )
    else:
        print(
            f"NO continuation portfolio={len(portfolio)} remain={remain:.1f}s closed_hits={n_closed}",
            flush=True,
        )

    stop_reason = None if cont is None else cont.stop_reason
    interp = interpret_continuation_stop(stop_reason, solved=bool(cont.solved) if cont is not None else False)
    if cont is not None:
        max_f = max(max_f, int(cont.max_foundations or 0))

    improved = False
    replay_ok = False
    replay_g = None
    terminal_lineage = None
    if cont is not None and cont.solved and cont.solution_actions and int(cont.solution_g or 10**9) < 187:
        g129 = reconstruct_g128_root(opening)
        prefix = as_actions((g129.get("post") or {}).get("full_actions") or [])
        recovered = None
        winner = None
        for rec in portfolio:
            ident = rec.get("ident") or rec.get("whole_game_identity")
            digest = rec.get("ordered_digest")
            if not digest:
                continue
            recovered = recover_digest_path(opening, g129["post"], digest, int(rec["g"]), time_s=120.0, unique=200_000)
            if recovered.get("ok"):
                winner = rec
                break
        if recovered and recovered.get("ok") and winner is not None:
            full = as_actions(recovered["hit"]["full_actions"]) + as_actions(cont.solution_actions)
        else:
            full = prefix + as_actions(cont.solution_actions)
        end = opening.clone()
        try:
            replay_g = replay_actions(end, list(full))
        except Exception:
            replay_g = None
        replay_ok = (
            replay_g == int(cont.solution_g)
            and end.is_solved()
            and sum(1 for a in full if is_deal(a)) == 5
            and len(end.foundations) == 8
            and not end.stock
            and all(c.is_empty() for c in end.columns)
        )
        if replay_ok:
            save_solution(full, FIX, g=int(cont.solution_g), label="Autonomous v0.83 F3 bridge calibration")
            terminal_lineage = foundation_progression(opening, list(full))
            _write_json(
                META,
                {
                    "g": int(cont.solution_g),
                    "replay_ok": True,
                    "canonical_input": False,
                    "parent_incumbent": 187,
                    "continuation_stop": cont.stop_reason,
                },
            )
            improved = True

    payload = {
        "experiment": EXPERIMENT,
        "base_sha": BASE_SHA,
        "branch": BRANCH,
        "policy_reads_canonical": False,
        "harvested": False,
        "incumbent_g": AUTONOMOUS_INCUMBENT_MW,
        "candidate_ceiling": CANDIDATE_CEILING,
        "record_mw": RECORD_MW_COST,
        "canonical_mw": CANONICAL_MW_COST,
        "wall_s": time.perf_counter() - wall0,
        "g141_first_viable_s": G141_FIRST_VIABLE_S,
        "stage_a_s": STAGE_A_S,
        "roots": [slim_root(r) for r in checked["roots"]],
        "controls": {"g141": G141_CONTROL, "g158": G158_CONTROL},
        "stage_a": [slim_agg(a) for a in stage_a],
        "no_stage_b": True,
        "bridge_table": bridge_table,
        "probe_table": probe_table,
        "bridge_summary": bridge_summary,
        "g160_vs_g161": g160_vs_g161,
        "composition_differs": composition_differs,
        "portfolio_sources": sorted({int(r.get("root_g")) for r in portfolio if r.get("root_g") is not None}),
        "portfolio_targets": sorted({r.get("tactical_target") for r in portfolio if r.get("tactical_target")}),
        "n_raw": n_raw,
        "n_viable": n_viable,
        "n_surplus1": n_surplus1,
        "n_strong": n_strong,
        "n_closed": n_closed,
        "n_closed_table": len(closed),
        "best_new_f": best_new_f,
        "best_bridge_loss": best_loss,
        "screening_time_limited": screening_time_limited,
        "n_portfolio": len(portfolio),
        "portfolio": [slim_term(r) for r in portfolio],
        "elapsed_s": None if cont is None else cont.elapsed_s,
        "unique": None if cont is None else cont.unique,
        "expanded": None if cont is None else cont.expanded,
        "generated": None if cont is None else getattr(cont, "generated", None),
        "stop_reason": stop_reason,
        "roots_exhausted": bool(interp.get("exhausted")),
        "recommend_continue_from_these_roots": interp.get("recommend_continue_from_these_roots"),
        "continuation_note": interp.get("note"),
        "max_foundations": max_f,
        "solved": bool(cont.solved) if cont is not None else False,
        "solution_g": None if cont is None else cont.solution_g,
        "replay_ok": replay_ok,
        "replay_g": replay_g,
        "accounting_fail": bool(getattr(cont, "accounting_fail", False)) if cont is not None else False,
        "bound_perf": None if cont is None else {
            "prunes": cont.lower_bound_prunes,
            "calls": cont.lower_bound_calls,
        },
        "lanes": None if cont is None else {"expansions": cont.lane_exp},
        "terminal_lineage": terminal_lineage,
        "incumbent_updated": improved,
        "envelope": {
            "time_s": TOTAL_S,
            "stage_a_s": STAGE_A_S,
            "stage_b_s": 0.0,
            "n_roots": N_ROOTS,
            "continuation_reserve_s": CONTINUATION_RESERVE_S,
            "ceiling": BRIDGE_CEILING,
            "rss_abort_mb": SEARCH_RSS_MB,
        },
    }
    verdict, reason = choose_calibration_verdict(payload)
    payload["verdict"] = verdict
    payload["verdict_reason"] = reason
    payload["interpretation"] = reason
    payload["next_recommendation"] = next_recommendation(verdict)
    payload = _jsonable(payload)
    _write_json(RESULT, payload)
    write_report(payload)
    _write_json(
        PROG,
        {
            "phase": "complete",
            "verdict": verdict,
            "best_new_f": best_new_f,
            "best_bridge_loss": best_loss,
            "stop_reason": stop_reason,
        },
    )
    print(f"VERDICT {verdict}", flush=True)
    print("DONE", flush=True)
    return payload


if __name__ == "__main__":
    main()

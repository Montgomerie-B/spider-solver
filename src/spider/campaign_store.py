...
def new_campaign(*, incumbent_g: int = 187, ceiling: int = 186) -> dict:
    return {
        "version": 1,
...
def add_candidate(data: dict, *, g: int, ordered_digest: str, label: Optional[str] = None) -> dict:
    rec = {
        "id": str(uuid.uuid4()),
        "g": int(g),
...
def enqueue_job(data: dict, candidate_id: str, *, ceiling: int, time_s: float, max_unique: int) -> dict:
    job = {
        "id": str(uuid.uuid4()),

"""v0.72 integrated strategic/tactical cash-out.
...
readiness_r2/r3 remain implemented historically but are not active.

At rows=1 only, after ordinary harvest, a small diverse set of ready
roots is probed with search_foundation_cashout. Successful target-
foundation terminals rejoin the pre-Deal candidate set. Deal-preview
and stock-empty assembly then decide continuation.
...
from spider.structural_analysis import _n
...
def select_tactical_probe_roots(
    ...
    *,
    ...
    """At most eight category-diverse ready rows=1 roots. Not the eight cheapest."""
...
def rows1_cashout_augment(rows: int, roots: Sequence[dict], budget_s: float, context: dict) -> dict:
    ...
    n_ready_roots = sum(
        1
        for rec in roots
        if int(rec.get("stock_rows") or -1) == 1 and int(rec.get("n_ready") or rec.get("n_ready_ranked") or 0) >= 1
    )
    probes = select_tactical_probe_roots(roots, limit=TACTICAL_ROOT_LIMIT, cost_ceiling=ceiling)
    if not probes:
    ...
    for i, root in enumerate(probes):
        ...
        if wall <= 0.02 or remaining_u <= 0:
            break
        share_s = max(0.05, wall / float(left))
        share_u = min(TACTICAL_PER_ROOT_UNIQUE, max(32, remaining_u // left))

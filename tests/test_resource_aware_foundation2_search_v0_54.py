...
def test_5_6_7_stock_zero_one_spade_foundation():
    src = inspect.getsource(load_deal_now_roots) + inspect.getsource(_replay := __import__(
        "spider.simple_resource_aware_f2", fromlist=["_replay_root"]
    )._replay_root)
    assert "stock_rows(end) == 0" in src
    assert "len(end.foundations) == 1" in src
...
def test_8_9_10_symmetry_cheapest_g_origin_not_identity():
    src = inspect.getsource(union_roots) + inspect.getsource(search_foundation2)
    ...
    ident_block = inspect.getsource(search_foundation2)
    assert "best_g[child_sym]" in ident_block
...
def test_11_12_13_ucs_g_all_tableau_no_deal():
    ...
    assert "heappush(heap, (child_g" in src or "heappush(heap, (g_of[node]" in src
...
def test_22_23_min_live_g_and_harvest_slack():
    ...
    assert "heap[0][0]" in src
    assert "current_incumbent + harvest_slack" in src
